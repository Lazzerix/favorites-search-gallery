import { EMPTY_POOL_ITEMS_PAGINATION_PARAMETERS, PoolItemsPaginationParameters } from "../../types/pool_items_pagination_parameters";
...
export function create(parameters: PoolItemsPaginationParameters): void {
  CONTAINER.innerHTML = "";
  updateRangeIndicator(parameters.startIndex, parameters.endIndex, parameters.poolItemsCount);
...
export function update(parameters: PoolItemsPaginationParameters): void {
  const atMaxPageNumberButtons = document.getElementsByClassName("pagination-number").length >= PoolItemsSettings.maxPageNumberButtons;
...
function createNumberTraversalButtons(currentPageNumber: number, finalPageNumber: number): void {
  const pageNumbers = getNumbersAround(currentPageNumber, PoolItemsSettings.maxPageNumberButtons, 1, finalPageNumber);
...
function createNumberTraversalButton(currentPageNumber: number, pageNumber: number): void {
  ...
  button.onclick = (): void => {
    Events.poolItems.pageSelected.emit(pageNumber);
  };
  CONTAINER.appendChild(button);
  button.textContent = String(pageNumber);
}

function createArrowTraversalButtons(parameters: PoolItemsPaginationParameters): void {
...
function createArrowTraversalButton(name: PoolItemsPageRelation, textContent: string, position: InsertPosition): HTMLButtonElement {
  const button = document.createElement("button");
  ...
  button.onclick = (): void => {
    Events.poolItems.relativePageSelected.emit(name);
  };
...
function createGotoSpecificPageInputs(finalPageNumber: number): void {
  ...
  button.onclick = (): void => {
    if (isOnlyDigits(input.value)) {
      Events.poolItems.pageSelected.emit(Number(input.value));
    }
  };
...
function updateArrowTraversalButtonInteractability(previousPage: HTMLButtonElement, firstPage: HTMLButtonElement, nextPage: HTMLButtonElement, finalPage: HTMLButtonElement, parameters: PoolItemsPaginationParameters): void {
...
export function setupPoolItemsPaginationMenu(): void {
  insert();
  create(EMPTY_POOL_ITEMS_PAGINATION_PARAMETERS);
