...
export async function updateMissingMetadata(): Promise<void> {
  ...
  const chunks = splitIntoChunks(FETCH_UPDATE_QUEUE, POOL_ITEMS_PER_PAGE).filter(chunk => chunk.length > 0);
...
export class FavoriteMetadata {
  public metrics: PoolItemMetricMap;
  public id: string;
  public rating: Rating;
  public isDeleted: boolean;

  constructor(id: string, record: PoolItemsDatabaseRecord | HTMLElement) {
    this.metrics = {
      id: parseInt(id),
      width: 0,
      height: 0,
  ...
  public get databaseRecord(): PoolItemsMetadataDatabaseRecord {
    return {
  ...
  public populate(object: PoolItemsDatabaseRecord | HTMLElement): void {
  ...
  public updateDatabase(): void {
    Events.poolItems.missingMetadataFound.emit(this.id);
  }
  ...
  private populateFromDatabase(record: PoolItemsDatabaseRecord): void {
