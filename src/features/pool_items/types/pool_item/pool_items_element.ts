...
import { ON_DESKTOP_DEVICE, USER_IS_ON_THEIR_OWN_POOL_PAGE } from "../../../../lib/global/flags/intrinsic_flags";
import { createObjectURLFromSvg, openOriginal, openPostPage } from "../../../../utils/dom/links";
import { Events } from "../../../../lib/global/events/events";
import { FavoriteElement } from "./pool_items_types";
...
export function createPoolItemHTMLTemplates(): void {
...
function createPostHTMLTemplate(): void {
  ...
  htmlTemplate.innerHTML = `
        <a>
          <img>
          ${USER_IS_ON_THEIR_OWN_POOL_PAGE ? REMOVE_FAVORITE_IMAGE_HTML : ADD_FAVORITE_IMAGE_HTML}
...
export class FavoriteHTMLElement implements FavoriteElement {
  ...
  constructor(post: Post) {
    ...
    this.setupFavoriteButton(USER_IS_ON_THEIR_OWN_POOL_PAGE);
  ...
  public swapFavoriteButton(): void {
    const isRemoveButton = this.favoriteButton.classList.contains("remove-pool_item-button");
  ...
  private removeFavorite(): void {
    Events.poolItems.poolItemRemoved.emit(this.root.id);
